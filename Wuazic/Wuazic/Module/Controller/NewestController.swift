...
class NewestController: BaseController {
    
    let column: CGFloat = UIDevice.current.isiPhone ? 3 : 7
    ...
    fileprivate let listCollectionView: UICollectionView = {
        ...
        view.backgroundColor = .clear
        view.registerItem(cell: FeatNewestItemCell.self)
...
extension NewestController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if indexPath.section == 0 {
            ...
            let cell: FeatNewestItemCell = collectionView.dequeueReusableCell(indexPath: indexPath)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if indexPath.section == 0 {
            ...
            AdsInterstitialHandle.shared.tryToPresent { [unowned self] in
                ...
                detail.newest = data[indexPath.row]
...
extension NewestController: UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if indexPath.section == 0 {
            ...
            let size = FeatNewestItemCell.size(height: collectionView.size.height)
            let width: CGFloat = ((collectionView.size.width - (column + 1) * kPadding) / column).rounded(.down)
            let height: CGFloat = width * size.height / size.width

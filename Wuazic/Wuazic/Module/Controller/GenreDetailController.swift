...
class GenreDetailController: BaseController {
    ...
    fileprivate let listCollectionView: UICollectionView = {
        ...
        view.backgroundColor = .clear
        view.registerItem(cell: GenreDetailCell.self)
...
extension GenreDetailController: UICollectionViewDelegate, UICollectionViewDataSource {
    func presentGenreDetail(_ g: GenreObject) {
        AdsInterstitialHandle.shared.tryToPresent { [unowned self] in
            let detail = GenreDetail1Controller()
            detail.genre = g
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if indexPath.section == 0 {
            ...
            let cell: GenreDetailCell = collectionView.dequeueReusableCell(indexPath: indexPath)
            cell.detail = details[indexPath.row]
            cell.onMore = { [weak self] genreDetail in
                self?.presentGenreDetail(genreDetail.toGenre())
            }
...
extension GenreDetailController: UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        return GenreDetailCell.size(width: collectionView.size.width - 2 * kPadding)
    }
}

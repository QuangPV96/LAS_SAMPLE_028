...
enum PlaylistOption {
    case addTrack, share, rename, delete, favourite
    
    func info() -> (String, String, String) {
        switch self {
        case .addTrack: return ("ic-playlist-add", "", "Add tracks")
        case .share: return ("ic-share", "", "Share playlist with friends")
        case .rename: return ("ic-rename", "", "Rename playlist")
        case .delete: return ("ic-delete-from-playlist", "", "Delete playlist")
        case .favourite: return ("ic-like", "ic-liked", "Favourite")
        }
    }
}

class PlaylistOptionView: BaseView {
    
    let heightItem: CGFloat = 60
    var options: [PlaylistOption] = [.addTrack, .rename, .delete] {
        didSet { listTableView.reloadData() }
    }
    var onSelected: ((PlaylistOption) -> Void)?
    var playlist: PlaylistObject? {
        didSet {
            imageView.sd_setImage(with: playlist?.trackThumbnailUrl, placeholderImage: Thumbnail.playlist, context: nil)
            titleTrackLabel.text = playlist?.title
        }
    }
...
extension PlaylistOptionView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if options[indexPath.row] == .favourite, let pl = playlist {
            cell.iconImage.image = DBService.shared.existsPlaylistOnDb(pl.playlistId) ? UIImage(imgName: data.1) : UIImage(imgName: data.0)
        }
